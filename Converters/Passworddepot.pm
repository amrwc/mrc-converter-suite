...
package Converters::Passworddepot;
...
=head1 Password Depot converter module

=head2 Platforms

=over

=item B<macOS>: Initially tested with version 9.1.3

=item B<Windows>: Initially tested with version 8.1.1
...
B<macOS>: Select the C<< File > Export >> menu item.
Enter your master password in the dialog when asked, and press C<OK>.
Leave the C<Export Format> as C<Password Depot XML Format *.xml>.
Click the C<...> icon at the far right of the C<Target File> area, navigate to your B<Desktop> folder, and set the C<Save As> name
to B<pm_export.xml>.
Click C<Continue>, and then click C<Close>.
Your XML text export file with the name you provided above is now on your Desktop.

B<Windows>: On the left side, select your password file's name (or the highest level folder you want to export).
Select the C<Tools> ribbon menu item, click the C<Export> button, and select the C<Export Wizard> item.
Enter your Password Depot password when the dialog appears, and press C<OK>.
The C<Export format> should be left as C<Extensible Markup Language format (*.xml)>.
Click the C<Browse> button, navigate to your B<Desktop> folder and in the C<File name> area, enter the name B<pm_export.xml>.
Be sure the C<Original folder> pulldown has the top-level folder you want exported (the top level is the entire contents of the password file).
Click C<Next> to complete the export and then click C<Finish> to close out the wizard.
Your XML text export file with the name you provided above is now on your Desktop.
...
Password Depot supports formatted text in Information items on Windows.
It does not, however, export this formatting information - only the raw text is exported.

Password Depot does not export your attachments.
Be sure to save them from within Password Depot before you remove it from your system.
