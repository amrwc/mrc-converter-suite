...
=head1 Handy Safe converter module

=head2 Platforms

=over

=item B<macOS>: Initially tested with version 1.02

=item B<Windows>: Initially tested with version Desktop Professional 3.01
...
Select the HANDY SAFE (topmost) grouping in the Handy Safe left sidebar.
Export the database as an XML export file by selecting the C<< File > Export >> menu item.
Navigate to your B<Desktop> folder in the Export File dialog, and in the C<File name> area (Windows) or C<Save As> area (macOS),
enter the name B<pm_export.xml>.
Click C<Save>.
