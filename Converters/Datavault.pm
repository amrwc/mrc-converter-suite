...
=head1 DataVault converter module

=head2 Platforms

=over

=item B<macOS>: Initially tested with version 5.2.39

=item B<Windows>: Initially tested with version 5.1.32
...
Export its database to a text file using the C<Tools E<gt> Export> menu item.
Select the C<All Items (DVX, CSV)> choice, and click C<OK>.
In the C<Save As> dialog, navigate to your B<Desktop> folder, set the file to B<pm_export.csv>,
and leave the C<Save as type> set to C<CSV files (.csv files)>.
Save the export.
...
The DataVault CSV export is generally lossy.
It does not export the Category, Type, or Template information.

The DataVault CSV export on Windows is lossy.
The user interface accepts Unicode characters, however, only latin1 characters are exported to the CSV file.
Non-latin1 characters are transliterated.  For example, the character B<ş> becomes B<s>.

The CSV export for DataVault for macOS does not properly CSV-quote its data.
This will cause the converter to skip the problematic record and generate a warning.
You will need to manually enter any of these noted records.

DataVault has several essentially identical templates that are indistinguishable in the CSV export.
These are treated as identical converter input types, and are listed below:

=over

=item bank account, checking account     --> bankacct

=item business contact, personal contact --> contact

=item credit card, mastercard, visa      --> creditcard

=item business, financial                --> business
...
sub do_import {
    ...
    my $csv = Text::CSV->new ({
	    binary => 1,
	    sep_char => ",",
	    eol => 		  $^O eq 'MSWin32' ? "\x{0d}\x{0a}" : "\x{0a}",
    ...
    # record boundaries.  So convert intra-field line endings to 0x01 and patch them later.
    #
    if ($^O eq 'darwin') {
	$data =~ s/\x{0d}\x{0a}/\x{01}/gs;	# 0d 0a		--> 0d
    }
    else {
	$data =~ s/\x{0d}{2}\x{0a}/\x{01}/gs;	# 0d 0d 0a	--> 0d
