...
=head1 Yojimbo converter module
...
Select the C<Library> sidebar area, and select all of the entries you want exported.
Export the selected items using the C<< File > Export... >> menu.
In the C<Open> dialog, navigate to your B<Desktop> folder, and click the C<New Folder> button at the bottom left of the dialog.
Enter the folder name B<pm_export>, click C<Create>, and then click the C<Choose> button.
Use the folder name you supplied above on the command line.

You may now quit Yojimbo.

=head2 Notes

By default, Serial Number entries are assigned to 1Password's Software License category.
You may use the option C<--serial2note> to force them into the Secure Note category. 

The converter will ignore B<image> and B<web archive> types.

Formatted notes will be converted into plain text.

The converter cannot decrypt encrypted items.
Be sure your items are decrypted prior to export.

Yojimbo does not export a record's B<Tags>, B<Flagged> status, B<Labels>, or B<Collections>.
And for some types, it does not export the entry's B<Comments>.
