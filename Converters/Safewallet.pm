...
=head1 SafeWallet converter module

=head2 Platforms

=over

=item B<macOS>: Initially tested with version 1.2

=item B<Windows>: Initially tested with version 2.4.1.2, 3.0.7
...
B<macOS>: Export its database to an XML file using the C<< File > Export... >> menu.
Select the C<XML> tab, and click C<Export>.
Navigate to your B<Desktop> folder, and save the file with the name B<pm_export.txt> to your Desktop.

B<Windows>: Export its database to an XML file using the C<< File > Export Wallet >> menu.
Select the C<Export to XML file> button, and click C<Next>.
Click the C<Browse button>.
Navigate to your B<Desktop> folder, and save the file with the name B<pm_export.txt> to your Desktop.
Click the C<Finish> button and press C<OK> when the successful export dialog appears.
