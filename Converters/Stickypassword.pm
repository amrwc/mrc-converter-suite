...
package Converters::Stickypassword;
...
=head1 Sticky Password converter module

=head2 Platforms

=over

=item B<macOS>: N/A (export is an ambiguously-structured text dump)

=item B<Windows>: Initially tested with version 8.0.6.145
...
Export its database to an XML file using the C<Menu> pulldown in the upper right corner of the user interface.
Select the C<Export> menu item, and then select the C<Export All> item.
On the next dialog screen, select C<Sticky Password 6 XML> and click C<Next>.
Select C<Save to file>.
On the next dialog screen, navigate to your B<Desktop> folder, and in the C<File name> area, enter the file name B<pm_export>.
Click the C<Save> button.
Your export file will be on your Desktop with the name B<pm_export.xml>.
