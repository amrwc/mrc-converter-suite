...
package Converters::Licensekeeper;
...
=head1 LicenseKeeper converter module

=head2 Platforms

=over

=item B<macOS>: Initially tested with version 1.8.4 (1702)
...
Export its database as an XML export file using the C<< File > Export > Library to XML... >> menu item.
When the folder save dialog appears, select B<Desktop> from the sidebar, and then click the C<Choose Export Folder> button.
This will store the XML file inside a folder named B<LicenseKeeper Export>.
The file named B<LicenseKeeper.xml> inside that folder is your exported XML data.

You may now quit LicenseKeeper.

=head2 Notes

The converter will rename the exported, name-encoded attachments.
They will be prefixed by the item's Title, and can be found in the B<Attachments> folder within the B<LicenseKeeper Export> folder.
