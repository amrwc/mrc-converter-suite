...
=head1 SPB Wallet converter module
...
=item B<Windows>: Initially tested with version 2.1.2 (12118)
...
There is no need to export any data from SPB Wallet.
The converter will read and decrypt the data directly from the SPB Wallet B<.swl> file.
On the command line, provide the path to your .swl file as the file name to convert.

It might be easier if you move your .swl wallet file to your Desktop, and then you can
refer to it on the command line using the relative path such as I<..\name_of_your_spbwallet_file.swl>.
If you prefer not to move this .swl file, on the command line, you will need to specify a valid path to your wallet file.
