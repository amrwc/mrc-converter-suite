...
package Converters::Passwordagent;
...
=head1 Password Agent converter module
...
=item B<Windows>: Initially tested with version 2.6.3
...
Export its database as an XML file using the C<< File > Print & Export >> menu item.
When the C<Print & Export Wizard> dialog opens, click the C<all> selector above the C<Groups> listing,
and click the C<all> selector above the C<Fields> listing.
Under the C<Output format>, select the C<XML - export database> option.
You may ignore the C<Sort by> pulldown.
Click C<Next> to continue.
When the C<Save As> dialog appears, click the C<Browse> button, navigate to your B<Desktop>, and save the file
to your Desktop with the name B<pm_export.xml>.
When the export has completed, close the wizard.
